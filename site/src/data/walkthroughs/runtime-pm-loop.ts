...
# v1.3 protocol reference: skills/integrations-engineer/references/pm-mcp-protocol.md
...
# Required-audit transitions: deploy-ops will not transition into these
...
**Project:** Platform · **Cycle:** 2026-W19 · **Type:** Bug · **State:** Todo → In Progress (transition by git-ops at 09:31) → In Review → Done
...
2026-05-08 09:31:46  git-ops        state transitioned: Todo → In Progress
\`\`\`

Checkpoint reference: \`.checkpoints/monitoring-ops.checkpoint.json\` →
...
[Initially deferred 10:38Z — Linear returned 503 on save_comment.
 ...
 deploy-ops.checkpoint.json pm_flush_log[] for the audit trace.]
\`\`\`
...
\`\`\`
2026-05-08 10:08:14  deploy-ops     created (staging deploy starting)
2026-05-08 10:08:48  deploy-ops     staging build complete (34s)
2026-05-08 10:09:14  deploy-ops     migration applied (concurrent index build 41s)
2026-05-08 10:14:02  deploy-ops     state transitioned: → Staging verified
2026-05-08 10:38:08  deploy-ops     prod deploy invoked
2026-05-08 10:38:14  deploy-ops     prod-shipped comment DEFERRED (Linear 503)
2026-05-08 10:42:03  deploy-ops     /deploy --retry-pm flushed 1 action
2026-05-08 10:42:08  deploy-ops     linked-shipped comment posted on PLAT-5102
2026-05-08 10:42:11  deploy-ops     state transitioned: → Shipped
\`\`\`
...
- monitoring-sentinel: \`.checkpoints/monitoring-ops.checkpoint.json#sentinels.PLAT-5103\` (30-min armed)

Checkpoint: \`.checkpoints/deploy-ops.checkpoint.json\` → \`deploys.PLAT-5103\`.
`,
    },
    {
      id: "incident-ticket",
...
\`\`\`
<!-- opchain:monitoring-ops:burst-event:PLAT-5104:event-3 -->
...
10:12:02  monitoring-ops creates PLAT-5104; parent=PLAT-5103
10:12:02  monitoring-ops pages maya
...
10:31:29  monitoring-ops auto-resolves
10:31:30  state: Resolved (PM pending)
11:18:00  maya publishes postmortem
11:18:14  monitoring-ops opens PLAT-5105, PLAT-5106
11:18:15  state: Done
\`\`\`
...
Checkpoint: \`.checkpoints/monitoring-ops.checkpoint.json\` → \`incidents.PLAT-5104\`.
`,
    },
    {
      id: "checkpoint-trace",
...
**Protocol reference:** \`skills/integrations-engineer/references/pm-mcp-protocol.md\` §4
...
## 1. 10:38:14Z — deploy-ops queues a deferred action
...
opchain checkpoint show deploy-ops --field pm_deferred_actions
  ...
  "pm_user_facing_errors": [
    {
      ...
      "attempts": 1,
      "last_error": "Linear API returned 401: cross-team write not authorised (deploy-ops attempted to transition PLAT-5103 owned by team OPS, agent is scoped to team PLAT)",
      "retriable": false,
...
| deploy-ops | code-auditor.checkpoint.json | Read audit grade for the audit-gate decision (cached per-SHA) |
| git-ops | bug-check.checkpoint.json | Read pre-commit gate result |
| release-ops | (every skill).checkpoint.json | Aggregate "what shipped since last release" for /release plan |
...
Checkpoint reference for this run: \`.checkpoints/deploy-ops.checkpoint.json\`.
`,
    },
    {
      id: "audit-pipeline",
...
Checkpoint: \`.checkpoints/monitoring-ops.checkpoint.json\` → \`audit_pipeline_traces\`.
`,
    },
    {
      id: "release-stamp",
...
\`\`\`
<!-- opchain:git-ops:pr-opened:#2243 -->
...
Branch + commit + PR + comment + state transition. Five tool calls, zero retries. Handing off to code-auditor + bug-check then to deploy-ops for staging.`,
    ...
    },
    {
      ...
      skill: "code-auditor",
      content:
`code-auditor /audit pre-deploy on PR #2243.
...
mcp__claude_ai_Linear__save_comment("PLAT-5102",
  body="<!-- opchain:code-auditor:audit-posted:#2243 -->
...
Bug-check returned PASS (1.1s). PR is mergeable; deploy-ops can take over.`
    },
    {
      type: "beat",
    ...
    },
    {
      ...
      skill: "deploy-ops",
      content:
`**Audit gate** (code-auditor + security-auditor + bug-check) → all PASS, cached per-SHA from /git-sync.
...
Auto-resolves at 10:31:29Z (queue drained; upstream partner-x recovered). Postmortem published 11:18Z; remediation sub-tickets PLAT-5105 + PLAT-5106 opened with markers \`<!-- opchain:monitoring-ops:remediation:PLAT-5104:item-1 -->\` and \`...:item-2 -->\`. PLAT-5104 transitions to \`Done\`.`,
