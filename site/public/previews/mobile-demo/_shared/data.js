...
window.WALKTHROUGHS = [
  // ─────────────────────────────────────────────────────────────
  {
    ...
    outputs: [
      ...
      { id: "live", label: "Staging URL", kind: "deploy",
        body: "heads-down-staging.workers.dev — 42KB worker, TTFB 38ms, smoke suite 18/18 green. Production one /launch away." },
    ],
    ...
    steps: [
      ...
      { type: "beat", label: "03 · Design", caption: "ux-engineer builds the style book", skills: ["ux-engineer"] },
      ...
      { type: "beat", label: "04 · Build", caption: "Generator → Evaluator sprint loop", skills: ["app-architect"] },
      { type: "exchange", role: "user", content: "Run all remaining sprints. /approve" },
      { type: "exchange", role: "claude", skill: "app-architect",
        content: "Sprint 2 Session timer        91/100\nSprint 3 Team board           93/100\nSprint 4 Push opt-in          88/100  (1 re-eval)\nSprint 5 Stripe + soft cap    90/100\nSprint 6 Polish + privacy     95/100\nAll green. Invoking git-ops." },

      { type: "beat", label: "05 · Ship", caption: "git-ops → deploy-ops", skills: ["git-ops", "deploy-ops"] },
      { type: "exchange", role: "claude", skill: "git-ops",
        content: "6 branches, 6 draft PRs, descriptions + test plans generated. Handing off to deploy-ops.",
        artifacts: ["prs"] },
      { type: "exchange", role: "claude", skill: "deploy-ops",
        content: "heads-down-staging.workers.dev deployed. 42KB worker, 38ms TTFB, smoke suite all green. CSP/HSTS/nosniff/Frame-Options present. Production one /launch away.",
        artifacts: ["live"] },
      { type: "exchange", role: "user", content: "Soaked fine overnight. /launch" },
      { type: "exchange", role: "claude", skill: "deploy-ops",
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    outputs: [
      ...
      { id: "score", label: "Evaluator score", kind: "report",
        body: "ux-engineer grade: 92/100. Density up 3.2×. Time-to-first-insight: 47s → 6s on mobile. Color count 9 → 3. Single-sprint implementation behind feature flag FLAG_DASH_OKR." },
    ],
    ...
    steps: [
      ...
      { type: "exchange", role: "claude", skill: "ux-engineer",
        content: "This is a density problem, not a visual-design problem. 14 charts at equal weight, 9 hues, redundant encoding. Handing off to dash-forge — the Tufte-density specialist. The rest of your style book stays intact.",
        artifacts: ["audit"] },
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    outputs: [
      ...
      { id: "audit", label: "Audit gate report", kind: "audit.md",
        body: "code-auditor extra-strict pass on the new feature. Settlement model untouched (verified by AST diff). 0 N+1 queries. 24 tests added. 100% line coverage on new code. Merge approved." },
    ],
    ...
    steps: [
      ...
      { type: "exchange", role: "claude", skill: "app-architect",
        content: "Reading reverse-spec output as Phase-1 baseline. Carrier Scorecard scoped surgically: read-only over existing Bid/Shipment/Document tables. Zero touch to settlement.rb. New routes namespaced under /api/v1/scorecards. Behind FLAG_CARRIER_SCORECARD.",
        artifacts: ["feature"] },
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    outputs: [
      ...
      { id: "audit", label: "code-auditor money pass", kind: "audit.md",
    ...
    skills: ["integrations-engineer", "code-auditor", "deploy-ops"],
    runtime: "5d / 4h chat",
    steps: [
      ...
      { type: "beat", label: "02 · Contract", caption: "design the boundary before any code", skills: ["integrations-engineer"] },
      { type: "exchange", role: "claude", skill: "integrations-engineer",
      ...
      { type: "beat", label: "03 · Scaffold + wire", caption: "boundary layer first, live keys last", skills: ["integrations-engineer"] },
      { type: "exchange", role: "claude", skill: "integrations-engineer",
      ...
      { type: "beat", label: "04 · Audit gate", caption: "code-auditor before merge", skills: ["code-auditor"] },
      { type: "exchange", role: "claude", skill: "code-auditor",
      ...
      { type: "beat", label: "05 · Go-live", caption: "feature flag flipped", skills: ["integrations-engineer", "deploy-ops"] },
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    summary:
      "A live Workers app outgrew D1. migration-ops writes the cutover plan, deploy-ops runs it, monitoring-ops watches.",
    inputs: [
    ...
    outputs: [
      ...
      { id: "verify", label: "Verification gates", kind: "checks.md",
        body: "monitoring-ops watches p99 latency, error rate, row-count drift per phase. Drift > 0.1% → Slack alert. p99 regression > 20ms → auto-rollback to prior phase. Backfill parity: row counts + md5(concat(columns)) must match per table." },
    ...
    skills: ["migration-ops", "deploy-ops", "monitoring-ops", "orchestrator"],
    runtime: "1 Wednesday",
    steps: [
      ...
      { type: "beat", label: "02 · Execute + verify", caption: "deploy-ops runs; monitoring-ops watches", skills: ["deploy-ops", "monitoring-ops"] },
      { type: "exchange", role: "claude", skill: "deploy-ops",
      ...
      { type: "exchange", role: "user", content: "/ops status" },
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      { type: "exchange", role: "claude", skill: "code-auditor",
        content: "Chained from security-auditor. RLS bypass (Finding #1) is reachable from 4 routes — confirmed, not theoretical. No SQL injection. No hardcoded secrets in repo. 1 weak randomness usage in /invite (Math.random) — flagged as P2." },
      ...
      { type: "beat", label: "04 · Remediation + deploy gate", caption: "security-auditor → build pipeline", skills: ["security-auditor", "deploy-ops"] },
      { type: "exchange", role: "claude", skill: "security-auditor",
        ...
        artifacts: ["backlog"] },
      { type: "exchange", role: "claude", skill: "deploy-ops",
        content: "Next prod deploy gated on both: code-auditor green AND security-auditor backlog P0/P1 closed. Deploy pipeline updated. .checkpoints/security-auditor.checkpoint.json written." },
    ],
  },
];
