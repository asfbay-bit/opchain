...
## app-architect Phase 6 build loop (formerly `tri-dev`)

> **tri-dev is retired.** Its Generator→Evaluator build harness now lives inside
> **app-architect Phase 6**. The checkpoint guidance below still applies — read
> "the skill" as app-architect, and `/td-status` / `/td-build` as app-architect's
> `/status` / `/build`.
...
3. `/build` checks for checkpoint on entry — if mid-sprint, resume from last eval
...
- [ ] Cross-skill reads work (e.g., deploy-ops can read app-architect's checkpoint)
