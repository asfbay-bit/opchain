...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  /checkpoint          Show orchestrator state (registry + session, from the tracked checkpoint)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Type any command, or just describe what you need.
```

> **What's wired vs. what's a pattern.** The orchestrator is a *reasoning* skill —
...
> update | done | init`. `/ops status`, `/ops next`, and `/ops blockers` lean on
> those directly. `/ops register`, `/ops switch`, `/ops scan`, `/ops route`,
> `/ops health`, `/ops skills`, and the v1.2 PM verbs (`/ops resume`, `/ops ticket`,
> `/ops pm-status`) are **agent behaviors** — described here as patterns, executed by
...
`checkpoint-protocol` is not directly invocable — instead run its scaffolder:

```bash
node scripts/checkpoint.mjs init
```

That creates `.checkpoints/` + a starter README. Follow the checkpoint-protocol
...
checkpoint protocol tracks `.checkpoints/` in git, and the orchestrator persists the
**same way every other skill does**: a single tracked file at

```
.checkpoints/orchestrator.checkpoint.json
```
...
On session start the orchestrator reads its own checkpoint for the registry, then
...
with `project: "gtrackr"`, and let the orchestrator group by that field. This keeps
...
`/ops scan` searches **roots derived at runtime** — never a hardcoded path. In
...
> project, `node scripts/checkpoint.mjs next` *is* `/ops next` minus the
> cross-project layer — so the two never diverge. The orchestrator adds the
...
ECOSYSTEM HEALTH
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ✅ app-architect     vX.Y.Z   SKILL.md ✓  references ✓ (in sync)
  ✅ code-auditor      vX.Y.Z   SKILL.md ✓  references ✓ (in sync)
  ✅ orchestrator      vX.Y.Z   SKILL.md ✓  (self)
...
The orchestrator stores its own state in `.checkpoints/orchestrator.checkpoint.json`,
...
```jsonc
{
  ...
  "active_project": "aidops-core",       // starts as default each session; /ops switch changes it
  "last_scan": "2026-04-21T15:30:00Z",
  "scan_summary": {
    "aidops-core": { "skills_found": ["reverse-spec", "app-architect", "code-auditor"], "blocker_count": 1, "status_summary": "Sprint 2/4, 1 blocker" }
  },
  "routing_history": [
    { "at": "2026-04-21T15:30:00Z", "intent": "audit the code", "routed_to": "code-auditor", "project": "aidops-core" }
  ]
}
```
...
1. Read `.checkpoints/orchestrator.checkpoint.json` → `skill_state.registry`.
...
All writes go to the one tracked checkpoint (`update orchestrator --skill_state…`):
...
| `/ops status` scanned | `last_scan`, `scan_summary` |
| Routing dispatched | append to `routing_history` |

### `/checkpoint` Behavior

Like every other skill, the orchestrator has one tracked checkpoint. `/checkpoint`
summarizes its `skill_state`:

```
ORCHESTRATOR STATE  (.checkpoints/orchestrator.checkpoint.json — tracked in git)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Registry:
...
> **Two different things share the word "orchestrator" — don't conflate them:**
> - **`orchestrator` (this skill)** — `/ops`: multi-project registry, cross-project
